...
before(async () => {
    ...
    let safuFactoryFactory = new ethers.ContractFactory(
        factoryJson.abi,
        factoryJson.bytecode
    );
    safuFactory = await safuFactoryFactory
        .connect(admin)
        .deploy(await admin.getAddress());

    let safuRouterFactory = new ethers.ContractFactory(
        routerJson.abi,
        routerJson.bytecode
    );
    safuRouter = await safuRouterFactory
        .connect(admin)
        .deploy(safuFactory.address, weth.address);

    let safuMakerFactory = await ethers.getContractFactory("SafuMakerV2");
    safuMaker = await safuMakerFactory
        .connect(admin)
        .deploy(safuFactory.address, barAddress, safu.address, usdc.address);
    ...
    await safuRouter.connect(admin).addLiquidity(
        // creates pair
        usdc.address,
        safu.address,
    ...
    let safuPairFactory = new ethers.ContractFactory(
        pairJson.abi,
        pairJson.bytecode,
        admin
    ); // admin required for attach
    ...
    await safuPair
        .connect(admin)
        .transfer(safuMaker.address, precision.mul(10_000)); // 1% of LP
});
...
after(async () => {
    ...
    expect(
        (await usdc.balanceOf(await attacker.getAddress())).div(precision)
    ).to.be.gte(100 * 50);
    expect(
        (await safu.balanceOf(await attacker.getAddress())).div(precision)
    ).to.be.gte(100 * 50);
});
