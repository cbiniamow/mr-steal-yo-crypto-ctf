...
before(async () => {
	...
	optionsContract = await optionsContractFactory
		.connect(admin)
		.deploy(
			usdc.address,
			weth.address,
	...
	await weth
		.connect(adminUser2)
		.approve(optionsContract.address, ethers.constants.MaxUint256);
	let expiryTime =
		(await ethers.provider.getBlock("latest")).timestamp + 60 * 60 * 24 * 7; // one week
	...
	let createdOptionId = await optionsContract
		.connect(adminUser)
		.getLatestOptionId();
	await usdc
		.connect(adminUser)
...
it("solves the challenge", async function () {
	const hackFactory = await ethers.getContractFactory("SideEntranceHack");
	const hack = await hackFactory.connect(attacker).deploy();
	await hack.connect(attacker).run(usdcDaiPair.address);
});
