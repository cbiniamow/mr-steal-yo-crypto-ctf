...
before(async () => {
	...
	let addressRegistryFactory = await ethers.getContractFactory(
		"AddressRegistry"
	);
	...
	lockManager = await lockManagerFactory
		.connect(admin)
		.deploy(addressRegistry.address);

	let tokenVaultFactory = await ethers.getContractFactory("TokenVault");
	tokenVault = await tokenVaultFactory
		.connect(admin)
		.deploy(addressRegistry.address);

	let fnftHandlerFactory = await ethers.getContractFactory("FNFTHandler");
	fnftHandler = await fnftHandlerFactory
		.connect(admin)
		.deploy(addressRegistry.address);
	...
	await gov
		.connect(adminUser)
		.approve(revest.address, precision.mul(100_000));
	await revest
		.connect(adminUser)
		.mintAddressLock(
			...
			{
				asset: gov.address,
				depositAmount: precision.mul(1_000),
				depositMul: 0,
			}
		);
});
...
it("solves the challenge", async function () {
	const hackFactory = await ethers.getContractFactory("DegenJackpotHack");
	const hack = await hackFactory
		.connect(attacker)
		.deploy(revest.address, gov.address);
	await gov
		.connect(attacker)
		.approve(hack.address, ethers.constants.MaxUint256);
	await hack.connect(attacker).run();
});
...
after(async () => {
	...
	expect(await gov.balanceOf(await attacker.getAddress())).to.be.equal(
		precision.mul(100_001)
	);
});
