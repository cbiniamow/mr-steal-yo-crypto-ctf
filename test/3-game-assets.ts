...
before(async () => {
	...
	swordAsset = await swordAssetFactory
		.connect(admin)
		.deploy("SWORD", "SWORD");

	let shieldAssetFactory = await ethers.getContractFactory("GameAsset");
	shieldAsset = await shieldAssetFactory
		.connect(admin)
		.deploy("SHIELD", "SHIELD");
	...
	await swordAsset
		.connect(admin)
		.mintForUser(await adminUser.getAddress(), 1);
	await shieldAsset
		.connect(admin)
		.mintForUser(await adminUser.getAddress(), 1);

	expect(
		await swordAsset.balanceOf(await adminUser.getAddress())
	).to.be.equal(1);
	expect(
		await shieldAsset.balanceOf(await adminUser.getAddress())
	).to.be.equal(1);
});

it("solves the challenge", async function () {
	const hackFactory = await ethers.getContractFactory("GameAssetsHack");
	const hack = await hackFactory.connect(attacker).deploy();
	await hack
		.connect(attacker)
		.run(assetWrapper.address, swordAsset.address, shieldAsset.address);
});
...
after(async () => {
	...
	expect(
		await assetWrapper.balanceOf(await adminUser.getAddress(), 0)
	).to.be.equal(0);
	expect(
		await assetWrapper.balanceOf(await adminUser.getAddress(), 1)
	).to.be.equal(0);
});
